// EXTERNAL DEPENDENCIES
const passport = require("passport");

// INTERNAL DEPENDENCIES
const { catchAsync, catchSync } = require("../utils/catchers");

// MONGOOSE MODELS
const User = require("../models/user");

const showRegisterUserForm = catchSync((req, res, next) => {
  res.render("user/register", { name: "Register" });
});

const registerNewUser = catchAsync(async (req, res, next) => {
  try {
    const { email, username, password } = req.body;
    const newUser = new User({
      email,
      username,
    });
    const newUserRegistered = await User.register(newUser, password);
    req.login(newUserRegistered, (error) => {
      if (error) return next(error);
      console.log(newUserRegistered);
      req.flash("success", "¡Bienvenido!");
      res.redirect("/empleados");
      return;
    });
  } catch (error) {
    req.flash("error", error.message);
    res.redirect("/register");
  }
});

const authenticateUser = passport.authenticate("local", {
  failureFlash: true,
  failureRedirect: "/login",
});

const successfulUserAuthenticationRedirection = catchAsync(
  async (req, res, next) => {
    req.flash("success", "¡Bienvenido de regreso!");
    const redirectToUrl = res.locals.returnTo || "/empleados";
    delete res.locals.returnTo;
    res.redirect(redirectToUrl);
  }
);

const userLogout = catchSync((req, res, next) => {
  req.logout(function (err) {
    if (err) {
      return next(err);
    }
    req.flash("success", "See you later!");
    res.redirect("/");
  });
});

module.exports = {
  showRegisterUserForm,
  registerNewUser,
  authenticateUser,
  successfulUserAuthenticationRedirection,
  userLogout,
};
